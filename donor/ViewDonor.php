...
include "../function/Database.php";
 ...
 $target_page=0;
    ...
    {
      $target_page = ($get_page*2)-2;                       //ORDER BY donor_id
    }
    $query = "SELECT * FROM donor_tb WHERE status ='Accepted' ORDER BY donor_id limit $target_page,2 ";
    ...
    //    $query = "SELECT * FROM donor_tb where name='$_POST[srcname]' ";
    //    $read = $db->select($query);
    // }
   // if (isset($_GET['st'])) {
    //    $query = "SELECT * FROM donor_tb";
    //    $read = $db->select($query);
    // }

    //   if (isset($_GET['st1'])) {
    //    $query = "SELECT * FROM donor_tb WHERE activity = 'active'";
    //    $read = $db->select($query);
    // }

    //    if (isset($_GET['st2'])) {
    //    $query = "SELECT * FROM donor_tb WHERE activity = 'deactive'";
    //    $read = $db->select($query);
    // }
...
<a href="http://localhost/EUDV/donor/ViewDonorPending.php?page=1" class="btn btn-warning">View All Pending Donor</a>
...
<td>
  ...
  <td><a class="btn btn-warning" href="DeleteViewDonor.php?donor_id=<?php echo $row['donor_id']; ?>">Delete</a></td>
...
<?php
$query3 = "SELECT * FROM donor_tb WHERE status ='Accepted' ";
...
{
 ?> <a href="http://localhost/EUDV/donor/ViewDonor.php?page=<?php echo $target;?>"> <?php echo "<span style='font-size:25px'  >$target </span> &nbsp &nbsp &nbsp" ?></a> <?php
